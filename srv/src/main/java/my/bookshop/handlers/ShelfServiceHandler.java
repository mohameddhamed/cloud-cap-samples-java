...
public class ShelfServiceHandler implements EventHandler {
  ...
  public void beforeUpdateShelf(Shelves shelf, CdsUpdateEventContext context) {
    ...
    if (shelfId != null) {
      //      CqnSelect query = Select.from(ShelfService_.SHELVES).where(o ->
      // o.get("ID").eq(shelfId));
      // We can also do a byId select which is more efficient and cleaner
      CqnSelect query = Select.from(ShelfService_.SHELVES).byId(shelfId);
    ...
    if (shelf.getName() != null) {
      ...
      CqnSelect query =
          //          Select.from(ShelfService_.SHELVES).where(o ->
          // o.get("name").eq(shelf.getName()));
          // using the generated name method which is type safe and cleaner
          Select.from(ShelfService_.SHELVES)
              .where(o -> o.name().eq(shelf.getName()).and(o.ID().ne(shelfId)));
    ...
    if (shelf.getCapacity() != null) {
      CqnSelect shelfBookQuery =
          Select.from(ShelfService_.SHELF_BOOKS)
              ...
              // using the generated name method which is type safe and cleaner
              .where(sb -> sb.shelf_ID().eq(shelfId));
  ...
  public void beforeCreateShelf(Shelves shelf) {
    ...
    CqnSelect query =
        ...
        // using the generated name method which is type safe and cleaner
        Select.from(ShelfService_.SHELVES).where(o -> o.name().eq(shelf.getName()));
  ...
  public void beforeCreateShelfBook(ShelfBooks shelfBook, CdsCreateEventContext context) {
    ...
    CqnSelect bookQuery =
        //        Select.from(ShelfService_.BOOKS).where(o ->
        // o.get("ID").eq(shelfBook.getBookId()));
        // using the generated byId method which is more efficient and cleaner
        Select.from(ShelfService_.BOOKS).byId(shelfBook.getBookId());
    ...
    CqnSelect shelfBookAssociationQuery =
        Select.from(ShelfService_.SHELF_BOOKS)
            .where(
                // using the generated name methods which are type safe and cleaner
                o -> o.shelf_ID().eq(shelfId).and(o.book_ID().eq(shelfBook.getBookId())));
    ...
    // using the generated byId method which is more efficient and cleaner
    CqnSelect shelfQuery = Select.from(ShelfService_.SHELVES).byId(shelfId);
  ...
  public void AddBookToShelf(AddBookToShelfContext eventContext) {
    ...
    if (persistencyCheck.first().isPresent()) {
      //  Shelf exists
      CqnSelect bookQuery = Select.from(ShelfService_.BOOKS).byId(bookid);
      ...
      if (bookCheck.first().isPresent()) {
        ...
        CqnSelect shelfBookQuery =
            Select.from(ShelfService_.SHELF_BOOKS)
                .where(o -> o.shelf_ID().eq(shelfid).and(o.book_ID().eq(bookid)));
